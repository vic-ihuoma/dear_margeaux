...
export function VariantForm({
  ...
  const validate = useCallback((): boolean => {
    ...
    if (formData.reorder_point.trim()) {
      const reorderValue = parseInt(formData.reorder_point, 10);
      if (
        isNaN(reorderValue) ||
        reorderValue < 0 ||
        !Number.isInteger(reorderValue)
      ) {
        errors.reorder_point =
          'Reorder point must be a non-negative whole number';
      }
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Parse reorder_point: empty string = null (no reorder alert), otherwise integer
    const reorder = formData.reorder_point.trim();
    const reorderPoint = reorder ? parseInt(reorder, 10) : null;
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-text-muted">
            Alert when stock falls below this level. Leave empty for default
            (5).
          </p>
        </div>
