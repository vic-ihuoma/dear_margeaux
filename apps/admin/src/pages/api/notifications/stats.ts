...
export const GET: APIRoute = async () => {
  ...
  try {
    const result = await client.getEmailSendStats();
    ...
    console.error('Failed to get email send stats:', error);

    const message =
      error instanceof Error ? error.message : 'Failed to get email send stats';
